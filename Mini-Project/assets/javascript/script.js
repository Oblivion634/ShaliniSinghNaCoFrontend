...
// Create the "Remove Item" button to be added below todo-list for smaller screens 
const removeTaskButton = document.createElement("button");
removeTaskButton.id = "removeTaskButton"; 
removeTaskButton.className = "btn btn-danger"; 
removeTaskButton.style.marginBottom = "15px"; 
removeTaskButton.innerHTML = "<b>Remove Item</b>"; 
...
function handleMediaChange(event) {
  if (event.matches) {
    ...
    buttons.insertBefore(removeTaskButton, buttons.firstChild); //Add the new remove button
  } else {
    // If viewport is > 767.5px (desktop view)
    moveToComplete.innerHTML = " <b> MOVE TO RIGHT &gt;</b>";
    moveToTodo.innerHTML = "<b> &lt; MOVE TO LEFT</b>";
    if (removeTaskButton.parentElement === buttons) {
      buttons.removeChild(removeTaskButton); // Remove the extra remove button 
    }
  }
}
...
removeTaskButton.addEventListener("click", remove);
